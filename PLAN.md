...
- info-verse.org domain registered and DNS configured (completed)
...
### Step 9.1 - Domain Acquisition & DNS Migration — DELETED

Step 9.2 (Search Engine Indexing) was also deleted. The user's budget only
allows one domain (info-verse.org), which is already registered with Google
Search Console. Existing `*.info-verse.org` subdomains serve as the site
structure. No additional domain purchases are planned.

---

### Step 9.3 - Social Media Presences

Status: Complete (code). Placeholder social profiles wired into all 6 child themes.
User must create actual accounts and replace placeholder URLs.
...
**Code changes (Step 9.3):**

- `openclaw-base/functions.php`: 4 new functions:
  - `openclaw_social_meta_tags()` — emits `<meta>` tags in `<head>` (og:url,
    og:title, twitter:site, og:profile:url, al:android/url, al:ios/url)
    hooked to `wp_head` at priority 2.
  - `openclaw_social_links_shortcode()` — renders `[openclaw_social_links]`
    shortcode with SVG icon buttons.
  - `openclaw_social_label()` — human-readable labels for platform handles.
  - `openclaw_social_svg()` — inline SVG paths for 8 platforms (X, Facebook,
    Instagram, LinkedIn, Reddit, Bluesky, YouTube, TikTok).
- `openclaw-base/parts/footer.html`: social links section inserted via
  `<!-- wp:html -->[openclaw_social_links]<!-- /wp:html -->`.
- `openclaw-base/style.css`: `.openclaw-social-links` styled as 32px circular
  SVG icon buttons with hover scale(1.1).
- 6 child theme `functions.php` files define `OPENCLAW_SOCIAL_PROFILES`
  constants with placeholder URLs:
  - **techtools**: X @TechToolsInfoVerse, LinkedIn tech-tools-infoverse
  - **gardening**: Facebook GardeningInfoVerse, Instagram gardening_infoverse
  - **dogs**: Facebook DogInfoVerse, Instagram dog_infoverse
  - **coffee**: Facebook CoffeeInfoVerse, Instagram coffee_infoverse
  - **boardgames**: X @BoardgamesInfoVerse, Reddit r/BoardgamesInfoVerse
  - **hub**: X @InfoVerseNet, LinkedIn info-verse-network, Bluesky info-verse.network
- `openclaw-hub/parts/footer.html`: full social links section with X, LinkedIn,
  Bluesky SVG icons (replaces single X text link).
...
- [X] Social icon links wired into footer templates (base + hub) — user must create real accounts and replace placeholder URLs.
- [X] Social OG/meta tags wired into `wp_head` via `openclaw_social_meta_tags()` — user must create real accounts for tags to resolve correctly.
...
**Completed (Python):**
- RSS feeds already work via Yoast (Atom at `/feed/`) — no changes needed.
- `_load_network_config()` and `_slug_from_host()` in `main.py` — loads `network_config.json` for cross-site host validation.
- `_validate_anchors()` extended to accept cross-site hosts from `network_config.json` production_hosts — prevents legitimate cross-site links from being stripped.

**Completed (Theme/PHP):**
- Newsletter signup form embedded in all child theme footers (Buttondown embed).
- Social sharing bar (`[openclaw_share_bar]` shortcode in `functions.php` + `single.html`).
- Cross-site "related" sidebar (`[openclaw_network_crosslink]` shortcode in `functions.php` + `single.html`).
...
**Verification:**
- [x] RSS feed URLs return valid Atom XML on all 7 deployed sites (Yoast).
- [x] `<link rel="alternate">` tag present in `<head>` of every deployed page (Yoast).
- [x] Newsletter signup form visible in the footer of all sites (Buttondown embed).
- [x] Social sharing buttons appear below article body on single-post pages (`[openclaw_share_bar]`).
- [x] Cross-site "from other Info Verse sites" block appears on single-post pages (`[openclaw_network_crosslink]`), fetching from other subsites via REST API (with RSS fallback and production URL support).
- [x] Share bar renders on single-post pages with X, Facebook, Reddit, LinkedIn, and Copy link buttons — verified on techtools.localhost:8088.
- [ ] All share buttons produce working pre-fill URLs (test each platform by clicking).
...
**Completed (Python):**
- `publisher.list_cross_site_posts(subsite_urls, limit)` — fetches recent posts from other subsites via REST API, fail-soft on failure, returns `{title, link, excerpt, source_subsite}`.
- `main.py` cross-site post fetching — loads network config, fetches from all other subsites, merges cross-site candidates at the end of `link_candidates` (same-site links preferred).
- `generator.py` system prompt — cross-site candidates tagged `[cross-site: hostname]`, LLM instructed to link 1-2 per 10 articles when genuinely relevant.
- `_build_linking_candidates_message()` — formats cross-site posts with `[cross-site: hostname]` prefix.
- `_validate_anchors()` — extended to accept production hosts from `network_config.json`.
- `website_memory/network_config.json` — defines all 5 subsite URLs and production hosts.
- `website_memory/network_calendar.json` — editorial calendar with seasonal topics and cross-site coordination opportunities.

**Pending (Theme/PHP):**
- Hub enrichment (front-page.html + footer.html) — "What is Info Verse?" section, social links, newsletter CTA.
- Article-level cross-links via `[openclaw_network_crosslink]` shortcode (functions.php).
...
- [x] Cross-site link infrastructure in place — `list_cross_site_posts()` fetches from all subsites, candidates tagged `[cross-site: hostname]`, LLM instructed to link 1-2 per 10 articles, anchors validated against production hosts.
- [ ] Cross-site links appear in ≥ 10% of articles (roughly 1 cross-site link per 10 articles) — requires actual articles to verify.
- [ ] Cross-site links are contextually relevant (manual spot-check).
- [x] Network calendar file exists at `website_memory/network_calendar.json` with seasonal topics and cross-site coordination opportunities.
...
### Step 9.8 — Analytics-driven topic generation

**Status:** Complete (code). **Dependency:** GA4 and Search Console API access (service account `.json` key files). **Budget:** $0 (Google APIs are free within quotas).

**Strategy:** Feed actual traffic data from GA4 and Search Console back into the openclaw agent's topic selection pipeline. Currently, topic discovery relies on trending signals (Reddit RSS + Google Suggest) which tell what people are asking about — but not what's already driving traffic to your sites. This step closes that loop.

**Two capabilities:**

1. **Traffic-aware category weighting:** When the agent picks a topic, it's told which categories are already getting traffic. It prioritizes growing categories that already have readers rather than spreading evenly across all categories.
2. **Follow-up post generation:** The agent identifies top-performing posts (by pageviews and Search Console impressions) and proposes follow-up content — Part 2, comparisons, "what's next" pieces that extend coverage of what's already working.

**Implementation:**

1. **Create `openclaw/analytics.py`** — New module following `trends.py`'s fail-soft pattern:
   - `fetch_ga4_pageviews(site_host, property_id, path_prefix)` → `[{path, pageviews}]`
   - `fetch_sc_clicks(site_host, site_url)` → `[{url, clicks, impressions, ctr, position}]`
   - `gather_analytics_signals(site_host)` → `{"ga4": [...], "sc": [...]}` (never raises)
   - Local caching: `website_memory/{host}.analytics.json` with 24h TTL (skip API calls if cache is fresh)

2. **Add config fields** to `Config`:
   - `GA4_PROPERTY_ID` (numeric, e.g. "123456789" from `G-EMJRNCZR10`)
   - `GA4_SERVICE_ACCOUNT_KEY` (path to OAuth `.json` key file)
   - `SC_SITE_URL` (e.g. `"https://dogs.info-verse.org"`)
   - `SC_SERVICE_ACCOUNT_KEY` (path to OAuth `.json` key file)

3. **Wire into `main.py`** — Add `_load_analytics_config()` (analogous to `_load_trends_config()`), pass `analytics_signals` into `generate_article()` and `propose_topics()`.

4. **Modify `generator.py`** — Add `analytics_signals` parameter to `propose_topics()` and `generate_article()`. Add `_build_analytics_message()` that injects traffic data into the topic prompt:
   - "Top-performing categories by pageviews: X, Y, Z"
   - "Top posts by pageviews: [title] ([path]) — N pageviews"
   - "Posts with highest Search Console impressions: [title] — N impressions"

5. **Update `requirements.txt`** — Add `google-analytics-data` and `google-api-python-client`.

6. **Per-site GA4 filtering** — All 6 sites share property `G-EMJRNCZR10`. Filter by `hostname` dimension in the GA4 Data API (`dimensionFilters.hostname == dogs.info-verse.org`) rather than creating separate properties.

**Verification checklist:**

- [x] Code implemented: `openclaw/analytics.py`, config fields, main.py wiring, generator.py wiring, requirements.txt
- [x] `pip install google-analytics-data google-api-python-client` succeeds (2026-07-29)
- [x] `openclaw.analytics.gather_analytics_signals("dogs.info-verse.org")` returns `{"ga4": [], "sc": []}` with no API keys (fail-soft verified 2026-07-29)
- [x] analytics.py decoupled from `Config.load()` — reads `GA4_*` / `SC_*` from `os.environ` directly, so it works without bare WP_* vars
- [ ] `openclaw.analytics.gather_analytics_signals("dogs.info-verse.org")` returns `{"ga4": [...], "sc": [...]}` with real API keys (awaiting user-provided .json keys)
- [ ] Cache file `website_memory/dogs.info-verse.org.analytics.json` is created and reused for 24h (needs real API keys to populate)
- [ ] `propose_topics(analytics_signals=...)` produces topics that reference high-traffic categories
- [ ] `propose_topics(analytics_signals=...)` proposes follow-up topics based on top posts
- [ ] `python -m openclaw post --site dogs` generates an article reflecting traffic-aware category selection
- [ ] No API errors propagate into the publish path (fail-soft verified)

---

### Step 9.9 — Content expansion (deferred)

**Status:** Deferred. **Dependency:** Phase 9.8 analytics data (need traffic signals before expanding content). **Budget:** $0 (agent time only).

**Strategy:** Once 9.8 is operational and sites have measurable traffic, use actual pageview data to decide which categories and topics to expand. This is Phase Omega's core concern — writing more of what works. Currently deferred because analytics tuning without views is guesswork.

**Verification checklist:**

- [ ] Smoke test: a one-liner that fetches pageviews for one known URL from GA4
- [ ] `propose_topics(analytics_signals=...)` on a site with traffic data proposes topics weighted toward high-traffic categories
- [ ] Compare pre-9.8 vs post-9.8 topic distributions (more concentration in high-traffic categories after 9.8)
- [ ] Content expansion decisions are based on actual traffic data (not guesses).
...
- 2026-07-29: **analytics.py decoupled from Config.load()** — both `fetch_ga4_pageviews()` and `fetch_sc_clicks()` (and `gather_analytics_signals()`) previously called `Config.load()` to get GA4/SC env vars. Since `Config.load()` requires bare `WP_BASE_URL`/`WP_USERNAME`/`WP_APP_PASSWORD`, standalone calls to `gather_analytics_signals()` (e.g. smoke testing or offline use) would raise `RuntimeError`. Fixed by replacing all `Config.load()` calls in analytics.py with direct `os.environ.get()` reads of `GA4_PROPERTY_ID`, `GA4_SERVICE_ACCOUNT_KEY`, `SC_SITE_URL`, and `SC_SERVICE_ACCOUNT_KEY`. Also removed the `from .config import Config` import to fully eliminate the circular dependency.
