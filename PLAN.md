...
- 2026-06-18: SEO fields added to Claude tool schema — `excerpt` (150-160 char meta description), `slug` (short keyword URL), `focus_keyphrase` (2-4 word search phrase). `publisher.get_seo_plugin()` detects Yoast or RankMath from `/wp-json/` namespaces and routes the keyphrase to the correct meta key (`_yoast_wpseo_focuskw` / `rank_math_focus_keyword`). excerpt and slug are standard WP fields and always posted.
