...
def list_existing_focus_keyphrases(limit: int = 1000) -> list[str]:
    """Return distinct stored focus keyphrases across published posts and drafts.

    Focus keyphrases are SEO targets, so reusing one creates a stronger
    cannibalisation signal than merely reusing similar title wording. This is
    intentionally fail-soft: a site without the REST meta-registration plugin
    simply returns an empty list and continues using the existing title guard.
    """
    cfg = Config.load()
    auth = (cfg.WP_USERNAME, cfg.WP_APP_PASSWORD)
    keyphrases: list[str] = []
    ...
    while len(keyphrases) < limit:
        try:
            ...
            logger.warning("focus-keyphrase fetch failed: %s", exc)
            return keyphrases
        if not resp.ok:
            if resp.status_code == 400 and page > 1:
                break
            logger.warning("focus-keyphrase fetch returned %d; skipping keyphrase guard.", resp.status_code)
            return keyphrases
        batch = resp.json()
        if not batch:
            break
        for post in batch:
            ...
            value = meta.get("_yoast_wpseo_focuskw") or meta.get("rank_math_focus_keyword") or ""
            value = str(value).strip()
            normalized = value.casefold()
            if value and normalized not in seen:
                seen.add(normalized)
                keyphrases.append(value)
